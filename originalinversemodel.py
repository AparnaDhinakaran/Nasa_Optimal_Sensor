...
def inverse_model(sensor, num_sensors,cloudbins,bins_input):
    ...
    sensorss= num_sensors
    cloudinessbins= cloudbins
    bins= bins_input
    ...
    for clouds in range(len(getclouds)): #for each level of cloudiness
        ...
        print len(sunanglerange[clouds]),clouded,clouds
        y3=dict()
        y4=dict()
        #print (len(sunanglerange[clouds])-1) 
        if (range(len(sunanglerange[clouds]))[0] == 0):
            print "hi"
        else:
            angle = range(len(sunanglerange[clouds]))[0]
        #for angle in range(len(sunanglerange[clouds])-1):
            ...
            #savedata=open('C:\Users\chandrayee\Documents\GitHub\sensor-placement\\coefficients1_5_2\\'+filename,'a')
        ##PERFORM OLS
